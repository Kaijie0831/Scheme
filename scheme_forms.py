...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_procedure = do_lambda_form(lambda_form, env)
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        result = scheme_eval(expressions.first, env, expressions.rest == nil)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if len(clause) == 1:
                return test
            outcome, last = clause.rest, None
            while outcome is not nil:
                last = scheme_eval(outcome.first, env, outcome.rest == nil)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        name, expr = bindings.first.first, bindings.first.rest.first
